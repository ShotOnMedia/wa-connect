...
from app.models import Contact, ContactNote, ContactTag, ContactTagLink, Conversation, Message, User, UserRole
...
def _assert_contact_access(db,contact_id,user):
    if not _is_agent(user): return
    allowed=db.scalar(select(Conversation.id).where(Conversation.contact_id==contact_id,Conversation.assigned_user_id==user.id).limit(1))
    if not allowed: raise HTTPException(status_code=403,detail="This contact is not assigned to you")
def _detail(db,contact,user):
    conv_filter=[Conversation.contact_id==contact.id]
    if _is_agent(user): conv_filter.append(Conversation.assigned_user_id==user.id)
    conversation_count=db.scalar(select(func.count(Conversation.id)).where(*conv_filter)) or 0
    message_count=db.scalar(select(func.count(Message.id)).join(Conversation,Conversation.id==Message.conversation_id).where(*conv_filter)) or 0
    last_message_at=db.scalar(select(func.max(Conversation.last_message_at)).where(*conv_filter))
    db.refresh(contact)
...
def _get_detail(contact_id,db,user):
    ...
    _assert_contact_access(db,contact_id,user)
    return _detail(db,contact,user)

@router.get("",response_model=list[ContactListOut])
def list_contacts(q:str|None=Query(default=None,max_length=200),tag_id:int|None=Query(default=None),lifecycle:str=Query(default="active",pattern="^(active|archived|blocked|all)$"),db:Session=Depends(get_db),user:User=Depends(require_user)):
    stats_stmt=select(Conversation.contact_id.label("contact_id"),func.count(Conversation.id).label("conversation_count"),func.max(Conversation.last_message_at).label("last_message_at"))
    if _is_agent(user): stats_stmt=stats_stmt.where(Conversation.assigned_user_id==user.id)
    stats=stats_stmt.group_by(Conversation.contact_id).subquery()
    stmt=select(Contact,stats.c.conversation_count,stats.c.last_message_at).options(selectinload(Contact.tags)).join(stats,stats.c.contact_id==Contact.id) if _is_agent(user) else select(Contact,stats.c.conversation_count,stats.c.last_message_at).options(selectinload(Contact.tags)).outerjoin(stats,stats.c.contact_id==Contact.id)
...
def get_contact(contact_id:int,db:Session=Depends(get_db),user:User=Depends(require_user)): return _get_detail(contact_id,db,user)
@router.patch("/{contact_id}",response_model=ContactDetailOut)
def update_contact(contact_id:int,payload:ContactUpdate,db:Session=Depends(get_db),user:User=Depends(require_user)):
    contact=db.get(Contact,contact_id)
    if not contact: raise HTTPException(status_code=404,detail="Contact not found")
    _assert_contact_access(db,contact_id,user)
    if payload.name is not None: contact.name=payload.name.strip() or None;contact.updated_at=datetime.utcnow();db.commit()
    return _get_detail(contact_id,db,user)
...
def update_lifecycle(contact_id:int,payload:ContactLifecycleUpdate,db:Session=Depends(get_db),user:User=Depends(require_user)):
    ...
    contact.updated_at=now;db.commit();return _get_detail(contact_id,db,user)
@router.post("/{contact_id}/tags/{tag_id}",response_model=ContactDetailOut)
def add_contact_tag(contact_id:int,tag_id:int,db:Session=Depends(get_db),user:User=Depends(require_user)):
...
def remove_contact_tag(contact_id:int,tag_id:int,db:Session=Depends(get_db),user:User=Depends(require_user)):
    contact=db.get(Contact,contact_id)
    if not contact: raise HTTPException(status_code=404,detail="Contact not found")
    _assert_contact_access(db,contact_id,user)
