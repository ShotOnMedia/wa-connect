...
def list_api_keys(db: Session = Depends(get_db)):
    return [_key_out(row) for row in db.scalars(select(DeveloperApiKey).order_by(DeveloperApiKey.created_at.desc())).all()]
...
def create_api_key(payload: ApiKeyCreate, db: Session = Depends(get_db), user: User = Depends(require_admin)):
    ...
    if not workspace_id or not db.get(Workspace, workspace_id):
        raise HTTPException(status_code=400, detail="Create a workspace before generating an API key")
    scopes = sorted({s for s in payload.scopes if s in ALL_SCOPES})
    if not scopes:
        raise HTTPException(status_code=422, detail="Select at least one API scope")
    ...
    db.commit()
    db.refresh(row)
    return {**_key_out(row), "token": token}
...
def _field_rows(db: Session, workspace_id: int, contact_id: int, channel: str) -> dict[str, str | None]:
    ...
    rows = db.execute(
        select(ContactFieldDefinition.key, value_model.value_text)
        .outerjoin(value_model, (value_model.field_id == ContactFieldDefinition.id) & (value_model.contact_id == contact_id))
        .where(ContactFieldDefinition.workspace_id == workspace_id, ContactFieldDefinition.active.is_(True))
        .order_by(ContactFieldDefinition.sort_order, ContactFieldDefinition.id)
    ).all()
...
def _tag_rows(db: Session, workspace_id: int, contact_id: int, channel: str) -> list[dict]:
    ...
    rows = db.scalars(
        select(ContactTag)
        .join(link_model, link_model.tag_id == ContactTag.id)
        .where(link_model.contact_id == contact_id, ContactTag.workspace_id == workspace_id)
...
def _conversation_out(db: Session, channel: str, contact_id: int) -> dict | None:
    if channel == "telegram":
        row = db.scalar(select(TelegramConversation).where(TelegramConversation.contact_id == contact_id).order_by(TelegramConversation.last_message_at.desc(), TelegramConversation.id.desc()).limit(1))
        return None if not row else {"id": row.id, "status": row.status, "assigned_user_id": row.assigned_user_id, "last_message_at": row.last_message_at}
    row = db.scalar(select(Conversation).where(Conversation.contact_id == contact_id).order_by(Conversation.last_message_at.desc(), Conversation.id.desc()).limit(1))
    return None if not row else {"id": row.id, "status": row.status.value, "assigned_user_id": row.assigned_user_id, "last_message_at": row.last_message_at}


def _subscriber_out(db: Session, channel: str, row) -> dict:
    if channel == "telegram":
        name = " ".join(p for p in [row.first_name, row.last_name] if p).strip() or row.username
        subscriber_id = str(row.telegram_user_id)
        username = row.username
    else:
        name = row.name
        subscriber_id = row.wa_id
        username = None
    return {
        "id": f"{channel}:{row.id}",
        "channel": channel,
        ...
        "name": name,
        "username": username,
        "fields": _field_rows(db, row.workspace_id, row.id, channel),
        "tags": _tag_rows(db, row.workspace_id, row.id, channel),
        "conversation": _conversation_out(db, channel, row.id),
        "created_at": row.created_at,
...
def _subscriber(db: Session, ctx: DeveloperApiContext, ref: str, channel: str | None = None):
    ...
    row = db.scalar(select(model).where(model.id == ident, model.workspace_id == ctx.workspace_id))
    if not row:
...
def _subscriber_by_external_id(db: Session, ctx: DeveloperApiContext, channel: str, subscriber_id: str):
    if channel == "telegram":
        try:
            value = int(subscriber_id)
        except ValueError as exc:
            raise HTTPException(status_code=422, detail="Telegram subscriber_id must be the numeric Telegram user ID") from exc
    ...
    normalized = "".join(ch for ch in subscriber_id if ch.isdigit())
...
def _set_fields(db: Session, workspace_id: int, contact_id: int, channel: str, values: dict[str, Any]) -> dict[str, str | None]:
    value_model = TelegramContactFieldValue if channel == "telegram" else ContactFieldValue
    definitions = {f.key: f for f in db.scalars(select(ContactFieldDefinition).where(ContactFieldDefinition.workspace_id == workspace_id, ContactFieldDefinition.active.is_(True))).all()}
    unknown = sorted(k for k in values if k not in definitions)
    if unknown:
        raise HTTPException(status_code=422, detail=f"Unknown custom field key(s): {', '.join(unknown)}")
    for key, value in values.items():
        field = definitions[key]
        existing = db.scalar(select(value_model).where(value_model.contact_id == contact_id, value_model.field_id == field.id))
        ...
        if existing:
            ...
            db.add(value_model(contact_id=contact_id, field_id=field.id, value_text=text))
    db.flush()
    return _field_rows(db, workspace_id, contact_id, channel)
...
def _assign_tag(db: Session, workspace_id: int, contact_id: int, channel: str, tag_id: int | None = None, name: str | None = None):
    ...
    if not tag and name and name.strip():
        ...
        if not tag:
            tag = ContactTag(workspace_id=workspace_id, name=name.strip())
            db.add(tag)
            db.flush()
    if not tag or tag.workspace_id != workspace_id:
        raise HTTPException(status_code=404, detail="Tag not found")
    link_model = TelegramContactTagLink if channel == "telegram" else ContactTagLink
    exists = db.scalar(select(link_model.id).where(link_model.contact_id == contact_id, link_model.tag_id == tag.id))
    if not exists:
        db.add(link_model(contact_id=contact_id, tag_id=tag.id))
        db.flush()
    return tag
...
def _remove_tag(db: Session, contact_id: int, channel: str, tag_id: int):
    link_model = TelegramContactTagLink if channel == "telegram" else ContactTagLink
    db.execute(delete(link_model).where(link_model.contact_id == contact_id, link_model.tag_id == tag_id))
    db.flush()


def _log(db, ctx, request, started, code=200, channel=None):
    log_request(db, ctx, request, code, started, channel=channel)
...
def list_subscribers(
    ...
    if channel in (None, "telegram") and len(result) < limit:
        ...
        for row in db.scalars(stmt.order_by(TelegramContact.updated_at.desc()).limit(limit - len(result))).all():
...
def create_subscriber(request: Request, payload: SubscriberCreate, db: Session = Depends(get_db), ctx: DeveloperApiContext = Depends(require_scope("subscribers:write"))):
    ...
    if payload.channel == "telegram":
        raise HTTPException(status_code=422, detail="Telegram subscribers are created when they first interact with a connected bot; update an existing Telegram subscriber instead")
    wa_id = "".join(ch for ch in payload.subscriber_id if ch.isdigit())
    if not wa_id:
        raise HTTPException(status_code=422, detail="subscriber_id must contain a WhatsApp phone number")
    row = db.scalar(select(Contact).where(Contact.workspace_id == ctx.workspace_id, Contact.wa_id == wa_id))
    if not row:
        row = Contact(workspace_id=ctx.workspace_id, wa_id=wa_id, name=(payload.name or "").strip() or None)
        db.add(row); db.flush()
    elif payload.name is not None:
        row.name = payload.name.strip() or None
    if payload.fields:
        _set_fields(db, ctx.workspace_id, row.id, "whatsapp", payload.fields)
    for tag_name in payload.tags:
        _assign_tag(db, ctx.workspace_id, row.id, "whatsapp", name=tag_name)
    phone = db.scalar(select(WhatsAppPhoneNumber).join_from(WhatsAppPhoneNumber, Workspace, isouter=True)) if False else None
    if payload.connection_id:
        ...
        if phone and phone.account.workspace_id != ctx.workspace_id:
            phone = None
    if not phone:
    ...
    if phone and not db.scalar(select(Conversation.id).where(Conversation.contact_id == row.id, Conversation.phone_number_id == phone.id)):
        db.add(Conversation(workspace_id=ctx.workspace_id, phone_number_id=phone.id, contact_id=row.id))
    db.commit(); db.refresh(row)
    result = _subscriber_out(db, "whatsapp", row)
    _log(db, ctx, request, started, 201, "whatsapp")
    return result
...
def update_subscriber(request: Request, subscriber_ref: str, payload: SubscriberUpdate, db: Session = Depends(get_db), ctx: DeveloperApiContext = Depends(require_scope("subscribers:write"))):
    ...
    if payload.name is not None:
        if kind == "telegram":
            parts = payload.name.strip().split(" ", 1); row.first_name = parts[0] if parts else None; row.last_name = parts[1] if len(parts) > 1 else None
...
def delete_subscriber(request: Request, subscriber_ref: str, db: Session = Depends(get_db), ctx: DeveloperApiContext = Depends(require_scope("subscribers:write"))):
    started = time.perf_counter(); kind, row = _subscriber(db, ctx, subscriber_ref)
    if kind == "whatsapp":
        row.archived_at = datetime.utcnow(); row.updated_at = datetime.utcnow()
    else:
        db.delete(row)
...
def _flow_channel(db: Session, flow_id: int) -> str:
    return db.scalar(select(FlowChannelTarget.channel).where(FlowChannelTarget.flow_id == flow_id)) or "whatsapp"
...
async def trigger_bot_flow(request: Request, flow_id: int, payload: FlowTriggerRequest, db: Session = Depends(get_db), ctx: DeveloperApiContext = Depends(require_scope("flows:trigger"))):
    ...
    if channel == "telegram":
        ...
        conversation = None
        if payload.connection_id:
            conversation = db.scalar(select(Conversation).where(Conversation.contact_id == subscriber.id, Conversation.phone_number_id == payload.connection_id))
    ...
    result = {"ok": True, "flow_id": flow.id, "flow": flow.name, "channel": channel, "subscriber": f"{channel}:{subscriber.id}", "status": state, "waiting_for": waiting_for}
    _log(db, ctx, request, started, channel=channel); return result
