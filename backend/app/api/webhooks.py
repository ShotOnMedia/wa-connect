...
def _flow_reply_value(message):
    ...
    if str(message.message_type or "").lower() not in {"interactive", "button"}:return None
    ...
    if payload.get("type")=="button":
        button=payload.get("button") or {};return button.get("payload") or None
    interactive=payload.get("interactive") or {};reply_type=interactive.get("type")
    if reply_type=="button_reply":return (interactive.get("button_reply") or {}).get("id") or None
    if reply_type=="list_reply":return (interactive.get("list_reply") or {}).get("id") or None
    return None
...
def verify_webhook(hub_mode:str=Query(alias="hub.mode"),hub_verify_token:str=Query(alias="hub.verify_token"),hub_challenge:str=Query(alias="hub.challenge")):
    if hub_mode!="subscribe" or hub_verify_token!=settings.meta_verify_token:raise HTTPException(status_code=403,detail="Webhook verification failed")
    return Response(content=hub_challenge,media_type="text/plain")


@router.post("/whatsapp")
async def receive_webhook(request:Request,x_hub_signature_256:str|None=Header(default=None),db:Session=Depends(get_db)):
    raw_body=await request.body()
    if not verify_meta_signature(raw_body,x_hub_signature_256):raise HTTPException(status_code=401,detail="Invalid webhook signature")
    payload=await request.json();processed,inbound_messages=process_webhook_payload(db,payload);flows_executed=0
    for inbound_message in inbound_messages:
        original_body=inbound_message.body;capture=None;conversation=None
        try:
