...
def create_contact_field(payload:ContactFieldDefinitionCreate,db:Session=Depends(get_db),_:User=Depends(require_admin)):
    ...
    for workspace_id in _workspace_ids(db):
        field=ContactFieldDefinition(workspace_id=workspace_id,key=key,label=payload.label.strip(),field_type=payload.field_type,options_json=options,required=payload.required,active=payload.active,sort_order=payload.sort_order);db.add(field);db.flush()
