...
async def receive_telegram_webhook(bot_id: int, request: Request, x_telegram_bot_api_secret_token: str | None = Header(default=None), db: Session = Depends(get_db)):
    ...
    payload = await request.json(); update_id = int(payload.get("update_id") or 0)
    ...
    if callback:
        ...
        synthetic_id = -abs(update_id or int(datetime.utcnow().timestamp() * 1000))
        existing = db.scalar(select(TelegramMessage).where(TelegramMessage.conversation_id == conversation.id, TelegramMessage.telegram_message_id == synthetic_id))
        if existing:
            db.commit(); return {"ok": True, "processed": 0, "duplicate": True}
        ...
        db.add(inbound); conversation.last_message_at = timestamp; db.commit(); db.refresh(inbound)
        flows_executed = 0
        try:
            flows_executed = await run_telegram_flows_for_inbound(db, conversation, inbound); db.commit()
        except Exception:
    ...
    sender = message.get("from") or {}; chat = message.get("chat") or {}; telegram_message_id = message.get("message_id")
    _, conversation = _upsert_contact_conversation(db, bot, sender, chat)
    ...
    if existing:
        db.commit(); return {"ok": True, "processed": 0, "duplicate": True}
    message_type, body = detect_message_type(message); timestamp = datetime.utcfromtimestamp(message["date"]) if message.get("date") else datetime.utcnow()
    ...
    try:
        ...
        db.rollback(); logger.exception("Telegram flow runtime failed conversation=%s inbound=%s", conversation.id, inbound.id)
