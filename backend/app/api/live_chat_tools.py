...
def flows(channel:str,conversation_id:int,db:Session=Depends(get_db),user:User=Depends(require_user)):
    c=access(db,channel,conversation_id,user); workspace=wid(user,c)
    stmt=(select(Flow).join(FlowChannelTarget,FlowChannelTarget.flow_id==Flow.id)
          .where(Flow.workspace_id==workspace,Flow.status==FlowStatus.ACTIVE,FlowChannelTarget.channel==channel)
          .order_by(Flow.name))
    return [{"id":f.id,"name":f.name,"description":f.description,"trigger_type":getattr(f.trigger_type,"value",f.trigger_type)} for f in db.scalars(stmt).all()]
...
async def trigger(channel:str,conversation_id:int,payload:TriggerIn,db:Session=Depends(get_db),user:User=Depends(require_user)):
    c=access(db,channel,conversation_id,user); workspace=wid(user,c); f=db.get(Flow,payload.flow_id)
    target=db.scalar(select(FlowChannelTarget).where(FlowChannelTarget.flow_id==payload.flow_id))
    ...
    db.commit()
    return {"ok":True,"flow_id":f.id,"flow":f.name,"status":getattr(getattr(session,"status",None),"value",getattr(session,"status",None))}

class CannedIn(BaseModel):
    title:str;shortcut:str;body:str;channel:str="both";active:bool=True
...
def canned(channel:str,conversation_id:int,db:Session=Depends(get_db),user:User=Depends(require_user)):
    c=access(db,channel,conversation_id,user); workspace=wid(user,c)
    rows=db.scalars(select(CannedResponse).where(CannedResponse.workspace_id==workspace,CannedResponse.active.is_(True),or_(CannedResponse.channel=="both",CannedResponse.channel==channel)).order_by(CannedResponse.title)).all()
    return [{"id":r.id,"title":r.title,"shortcut":r.shortcut,"body":r.body,"channel":r.channel} for r in rows]

@router.get("/canned-responses/manage")
def manage(db:Session=Depends(get_db),user:User=Depends(require_user)):
    rows=db.scalars(select(CannedResponse).where(CannedResponse.workspace_id==user.workspace_id).order_by(CannedResponse.title)).all()
    return [{"id":r.id,"title":r.title,"shortcut":r.shortcut,"body":r.body,"channel":r.channel,"active":r.active} for r in rows]
...
def create(payload:CannedIn,db:Session=Depends(get_db),user:User=Depends(require_user)):
    ...
    if not shortcut or payload.channel not in {"both","whatsapp","telegram"}: raise HTTPException(422,"Invalid canned response")
    r=CannedResponse(workspace_id=user.workspace_id,title=payload.title.strip(),shortcut=shortcut,body=payload.body,channel=payload.channel,active=payload.active,created_by_user_id=user.id)
    db.add(r);db.commit();db.refresh(r);return {"id":r.id,"title":r.title,"shortcut":r.shortcut,"body":r.body,"channel":r.channel,"active":r.active}
