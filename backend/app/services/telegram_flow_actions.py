...
def _field_for_telegram_workspace(db,conversation,field_id):
    ...
    target=ContactFieldDefinition(workspace_id=conversation.workspace_id,key=source.key,label=source.label,field_type=source.field_type,options_json=source.options_json,required=source.required,active=source.active,sort_order=source.sort_order);db.add(target);db.flush();return target

def set_field(db,conversation,field_id,value):
    field=_field_for_telegram_workspace(db,conversation,field_id)
    if not field:return False
    text=None if value is None else str(value).strip();row=db.scalar(select(TelegramContactFieldValue).where(TelegramContactFieldValue.contact_id==conversation.contact_id,TelegramContactFieldValue.field_id==field.id))
...
def change_tag(db,conversation,tag_id,add):
...
def assign_user(db,conversation,user_id):
...
def condition_result(db,conversation,config):
    ...
    if field=='user_input':
        key,_,compare_value=str(config.get('value') or '').partition('\x1f');return compare(latest_input_answer(db,conversation.id,'telegram',key),compare_value,operator)
    ...
    if field=='custom_field':
        key=str(config.get('field_key') or config.get('key') or expected).strip();compare_value=str(config.get('compare_value') if 'compare_value' in config else ('' if key==expected else expected)).strip();row=db.execute(select(TelegramContactFieldValue.value_text).join(ContactFieldDefinition,ContactFieldDefinition.id==TelegramContactFieldValue.field_id).where(TelegramContactFieldValue.contact_id==conversation.contact_id,ContactFieldDefinition.workspace_id==conversation.workspace_id,ContactFieldDefinition.key==key)).first();return compare((row[0] if row else '') or '',compare_value,operator)
    return False
