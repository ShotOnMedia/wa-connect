...
from app.services.system_fields import sync_whatsapp_system_fields
...
def process_webhook_payload(db:Session,payload:dict)->tuple[int,list[Message]]:
    processed=0; inbound_messages=[]
    for entry in payload.get("entry",[]):
        for change in entry.get("changes",[]):
            value=change.get("value",{}); metadata=value.get("metadata",{}); meta_phone_number_id=metadata.get("phone_number_id")
            if not meta_phone_number_id: continue
            ...
            for item in value.get("messages",[]):
                ...
                if meta_message_id and db.scalar(select(Message.id).where(Message.meta_message_id==meta_message_id)): continue
                wa_id=item.get("from")
                if not wa_id: continue
                contact=db.scalar(select(Contact).where(Contact.workspace_id==workspace_id,Contact.wa_id==wa_id))
                profile_name=contacts_by_wa_id.get(wa_id)
                if not contact:
                    contact=Contact(workspace_id=workspace_id,wa_id=wa_id,name=profile_name); db.add(contact); db.flush()
                elif profile_name and contact.name!=profile_name: contact.name=profile_name
                location=item.get("location") if item.get("type")=="location" else None
                sync_whatsapp_system_fields(db,contact,profile_name,location)
            ...
            for status_payload in value.get("statuses",[]):
                ...
                if not meta_message_id or not status_value: continue
    ...
    db.commit(); return processed,inbound_messages
