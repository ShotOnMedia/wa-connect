...
def _field(db: Session, workspace_id: int, field_id: int | str | None = None, field_key: str | None = None):
    """Resolve by stable key. Legacy IDs from mirrored/global fields are translated to the same key in this workspace."""
    key = str(field_key or "").strip()
    if key:
        found = db.scalar(select(ContactFieldDefinition).where(ContactFieldDefinition.workspace_id == workspace_id,ContactFieldDefinition.key == key,ContactFieldDefinition.active.is_(True)))
        if found:return found
    if not field_id:return None
    try:fid=int(field_id)
...
def _path(value: Any, path: str):
    current=value;path=str(path or '').strip()
    if not path or path=='$':return current
    ...
    for part in [p for p in re.split(r'\.(?![^\[]*\])',path) if p]:
        ...
        if index is not None:
            if not isinstance(current,list) or int(index)>=len(current):return None
...
def _items(value: Any):
    ...
    if isinstance(value,dict):
        if value and all(str(k).isdigit() for k in value):return [value[k] for k in sorted(value,key=lambda k:int(k))]
        return list(value.values())
    return []


def _item_value(item: Any,path: str):
    if not str(path or '').strip():return item
...
def save_dynamic_selection(db: Session,channel: str,workspace_id: int,contact_id: int,config: dict,row: dict):
    ...
    else:db.add(model(contact_id=contact_id,field_id=target.id,value_text=text))
    db.flush();return True


def dynamic_selection_value(prefix: str,interactive_node_id: int,index: int)->str:return f'{prefix}:{interactive_node_id}:{index}'
def parse_dynamic_selection(value: str,prefix: str):
    match=re.fullmatch(rf'{re.escape(prefix)}:(\d+):(\d+)',str(value or '').strip());return (int(match.group(1)),int(match.group(2))) if match else None
