...
def _condition_matches(config,answers):
    rule=config.get('show_if') or {}
    key=str(rule.get('question_key') or '').strip()
    if not key:return True
    actual=answers.get(key)
    op=str(rule.get('operator') or 'equals').lower()
    expected=str(rule.get('value') or '')
    actual_text='' if actual is None else str(actual)
    a=actual_text.casefold();e=expected.casefold()
    if op=='equals':return a==e
    if op=='not_equals':return a!=e
    if op=='contains':return e in a
    if op=='not_contains':return e not in a
    if op=='is_empty':return actual is None or actual_text.strip()==''
    if op=='is_not_empty':return actual is not None and actual_text.strip()!=''
    return True

def _next_question(db,submission):
    rows=db.scalars(select(UserInputAnswer).where(UserInputAnswer.submission_id==submission.id,UserInputAnswer.campaign_question_id.is_not(None)).order_by(UserInputAnswer.id)).all()
    answered_ids={r.campaign_question_id for r in rows}
    answers={r.answer_key:r.value_text for r in rows}
    for question in _questions(db,submission.campaign_id):
        if question.id in answered_ids:continue
        if not _condition_matches(_question_config(question),answers):
            logger.info('Skipping Campaign question id=%s key=%s because show_if did not match',question.id,question.answer_key)
            continue
        return question
    return None
...
def _resolve_campaign(db,flow,conversation,config):
    ...
    for workspace_id in workspace_ids:
        ...
        if campaign:
            logger.info('Resolved Campaign %s id=%s via workspace=%s flow=%s conversation=%s',name,campaign.id,workspace_id,flow.id,conversation.id);return campaign
    matches=db.scalars(select(Campaign).where(Campaign.name==name).order_by(Campaign.id)).all()
    if len(matches)==1:
        campaign=matches[0];logger.warning('Resolved Campaign %s id=%s by unique-name fallback; flow workspace=%s conversation workspace=%s',name,campaign.id,getattr(flow,'workspace_id',None),getattr(conversation,'workspace_id',None));return campaign
...
async def _finish_wa(wa,db,flow,conversation,session,node,submission,config):
    await complete_submission(db,submission,config);thanks=str(config.get('thank_you_text') or '').strip()
...
def install():
    ...
    def start_with_campaign(original,db,flow,conversation,node,channel,config):
        campaign=_resolve_campaign(db,flow,conversation,config);submission=original(db,flow,conversation,node,channel,config)
        if campaign:
