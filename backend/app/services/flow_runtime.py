...
from app.flow_models import Flow, FlowEdge, FlowNode, FlowNodeType, FlowStatus, FlowStepType, FlowTriggerType
...
async def _execute_action(db: Session, conversation: Conversation, action_type: str, config: dict) -> None:
    ...
    if action_type == "delay":
        logger.info("Visual flow delay node skipped until durable flow runs are implemented")
        return
...
async def _run_graph(db: Session, flow: Flow, conversation: Conversation) -> bool:
    nodes=db.scalars(select(FlowNode).where(FlowNode.flow_id==flow.id)).all()
    if not nodes:return False
    edges=db.scalars(select(FlowEdge).where(FlowEdge.flow_id==flow.id).order_by(FlowEdge.sort_order,FlowEdge.id)).all()
    by_id={n.id:n for n in nodes}
    outgoing={}
    for e in edges:outgoing.setdefault(e.source_node_id,[]).append(e)
    current=next((n for n in nodes if n.node_type==FlowNodeType.TRIGGER),None)
...
async def run_flows_for_inbound(db: Session, conversation: Conversation, inbound_message: Message) -> int:
    ...
    for flow in matched:
        try:
            if await _run_graph(db,flow,conversation):
                db.commit();executed+=1;continue
            for step in sorted(flow.steps,key=lambda item:item.sort_order):
                await _execute_action(db,conversation,step.step_type.value,_json(step.config_json))
            db.commit();executed+=1
