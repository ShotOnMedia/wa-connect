...
def _media_value(message: Message) -> str:
    ...
    value = {"type": media_type,"id": media.get("id"),"mime_type": media.get("mime_type"),"sha256": media.get("sha256"),"caption": media.get("caption"),"filename": media.get("filename")}
...
def _validate_reply(config: dict, inbound_message: Message) -> tuple[bool, str | None, str | None]:
    ...
    expected_types = {"photo":"image","image":"image","audio":"audio","voice":"audio","video":"video","document":"document","file":"document","sticker":"sticker"}
    ...
    if reply_type=="email" and not re.fullmatch(r"[^\s@]+@[^\s@]+\.[^\s@]+",value):return False,None,custom_error or "Please enter a valid email address."
        ...
        if not re.fullmatch(r"\+?\d{7,15}",compact):return False,None,custom_error or "Please enter a valid phone number."
...
async def _send_flow_text(db: Session, conversation: Conversation, text: str) -> None:
    ...
    if contact.blocked_at:logger.warning("Flow send skipped: contact %s is blocked",contact.id);return
    if not service_window_open(conversation):logger.warning("Flow send skipped: conversation %s is outside the 24-hour service window",conversation.id);return
    ...
    meta_id=(response.get("messages") or [{}])[0].get("id");now=datetime.utcnow()
    db.add(Message(conversation_id=conversation.id,meta_message_id=meta_id,direction=MessageDirection.OUTBOUND,message_type="text",body=text,payload_json=json.dumps(response,ensure_ascii=False),status=MessageStatus.SENT,created_at=now));conversation.last_message_at=now;db.flush()
...
async def _capture_reply(db: Session, conversation: Conversation, waiting_node: FlowNode, inbound_message: Message) -> bool:
    ...
    if not valid:
        logger.info("Question node %s rejected reply type=%s",waiting_node.id,inbound_message.message_type);await _send_flow_text(db,conversation,validation_error or "Please try again.");return False
