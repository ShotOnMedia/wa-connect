...
VARIABLE_RE=re.compile(r'%([A-Za-z0-9_.-]+)%');INPUT_PREFIX='input.';CAMPAIGN_PREFIX='campaign.'
...
def render_whatsapp(db,conversation,text):
    ...
    field_keys={key for key in keys if not key.startswith((INPUT_PREFIX,CAMPAIGN_PREFIX))};values=_input_values(db,conversation.id,'whatsapp',keys);values.update(_campaign_values(db,conversation.id,'whatsapp',keys))
    if field_keys:
...
def render_telegram(db,conversation,text):
    value,keys=_keys(text)
    if not keys:return value
    field_keys={key for key in keys if not key.startswith((INPUT_PREFIX,CAMPAIGN_PREFIX))};values=_input_values(db,conversation.id,'telegram',keys);values.update(_campaign_values(db,conversation.id,'telegram',keys))
    if field_keys:
