...
async def _graph_get(path:str,access_token:str,params:dict|None=None)->dict:
    ...
    async with httpx.AsyncClient(timeout=20) as client:response=await client.get(url,headers=headers,params=params)
    if response.is_error:raise WhatsAppError(f"Meta API {response.status_code}: {response.text}")
...
async def _send_message(phone_number_id:str,access_token:str,payload:dict)->dict:
    ...
    async with httpx.AsyncClient(timeout=20) as client:response=await client.post(url,json=payload,headers=headers)
    if response.is_error:raise WhatsAppError(f"Meta API {response.status_code}: {response.text}")
    return response.json()
...
async def send_text_message(phone_number_id:str,access_token:str,to:str,text:str)->dict:
    return await _send_message(phone_number_id,access_token,{"messaging_product":"whatsapp","recipient_type":"individual","to":to,"type":"text","text":{"preview_url":False,"body":text}})
