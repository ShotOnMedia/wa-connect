...
A flow opts in through the durable ``Flow.interrupt_active_flow`` flag. No
...
from app.flow_models import Flow, FlowStatus, FlowTriggerType
...
def _interrupt_flow(db, workspace_id: int, channel: str, body: str | None) -> Flow | None:
    ...
    flows = db.scalars(
        ...
        .where(
            ...
            Flow.interrupt_active_flow.is_(True),
            FlowChannelTarget.channel == channel,
        )
        .order_by(Flow.id)
    ).all()
    for flow in flows:
        if matches(flow.trigger_value, text):
            return flow
    return None
...
def install() -> None:
    ...
    async def telegram_resume(db, conversation, inbound, session):
        ...
        session.status = "reset"; session.current_node_id = None; session.waiting_for = None; session.ended_at = datetime.utcnow(); session.updated_at = datetime.utcnow(); db.flush()
        ...
        try:
            ...
            if current:
                original_tg_track(new_run, current); current._interrupt_skip_run_id = old_run
            logger.info("Telegram interrupt flow matched conversation=%s previous_flow=%s interrupt_flow=%s keyword=%r", conversation.id, old_flow_id, flow.id, getattr(inbound, "body", None))
            return result
        except Exception as exc:
            current = tg._session(db, conversation.id)
            track_fail(new_run, exc, current.current_node_id if current else None)
            logger.exception("Telegram interrupt flow execution failed flow=%s conversation=%s", flow.id, conversation.id)
            if current: current._interrupt_skip_run_id = old_run
            return True

    def telegram_track(run_id, session):
        if getattr(session, "_interrupt_skip_run_id", None) == run_id:
            try: delattr(session, "_interrupt_skip_run_id")
    ...
    async def whatsapp_resume(db, conversation, inbound, session):
        ...
        if old_run:
            track_complete(old_run, f"Flow interrupted by keyword flow: {flow.name}")
        wa._finish(session, wa.FlowSessionStatus.RESET); session.updated_at = datetime.utcnow(); db.flush()
        ...
        try:
            ...
            original_wa_track(new_run, new_session); new_session._interrupt_skip_run_id = old_run
            logger.info("WhatsApp interrupt flow matched conversation=%s previous_flow=%s interrupt_flow=%s keyword=%r", conversation.id, old_flow_id, flow.id, getattr(inbound, "body", None))
            return result
        except Exception as exc:
            track_fail(new_run, exc, new_session.current_node_id); new_session._interrupt_skip_run_id = old_run
    ...
    tg._resume = telegram_resume; tg._track = telegram_track; wa._resume = whatsapp_resume; wa._track_state = whatsapp_track
    _installed = True
