...
def _matching_flows(db, conversation, inbound):
    flows = db.scalars(select(Flow).join(FlowChannelTarget, FlowChannelTarget.flow_id == Flow.id).where(
        Flow.workspace_id == conversation.workspace_id, Flow.status == FlowStatus.ACTIVE,
        FlowChannelTarget.channel == "telegram").order_by(Flow.id)).all()
    inbound_count = db.scalar(select(func.count(TelegramMessage.id)).where(
        TelegramMessage.conversation_id == conversation.id, TelegramMessage.direction == "inbound")) or 0
    matched = [flow for flow in flows if
        (_enum_value(flow.trigger_type) == FlowTriggerType.KEYWORD.value and _keyword_matches(flow.trigger_value, inbound.body)) or
        (_enum_value(flow.trigger_type) == FlowTriggerType.FIRST_MESSAGE.value and inbound_count == 1)]
    logger.info("Telegram flow match workspace=%s conversation=%s inbound=%r active=%s matched=%s",
                conversation.workspace_id, conversation.id, inbound.body, [f.id for f in flows], [f.id for f in matched])
    return matched
...
async def _send(db, conversation, text):
    ...
    db.add(TelegramMessage(conversation_id=conversation.id, telegram_message_id=int(result["message_id"]),
        direction="outbound", message_type="text", body=result.get("text") or text,
        payload_json=json.dumps(result, ensure_ascii=False), status="sent", telegram_timestamp=timestamp))
...
def _validate_question_reply(config, inbound):
    ...
    expected_types = {"photo":"photo","image":"photo","audio":"audio","voice":"voice","video":"video","document":"document","file":"document","sticker":"sticker"}
...
async def _run_from_node(db, flow, conversation, inbound, session, node, by_id, outgoing):
    ...
    while node and safety < 100:
        safety += 1
        session.current_node_id = node.id; session.status = "active"; session.waiting_for = None; session.updated_at = datetime.utcnow()
        config = _json(node.config_json); node_type = _enum_value(node.node_type)
        ...
        if node_type == FlowNodeType.SEND_MESSAGE.value:
            await _send(db, conversation, config.get("text")); node = _next(by_id, outgoing, node.id); continue
        if node_type == FlowNodeType.QUESTION.value:
            await _send(db, conversation, config.get("text")); session.status = "waiting"; session.waiting_for = "reply"; db.flush(); return True
        ...
        if node_type == FlowNodeType.SET_STATUS.value:
            set_status(db, conversation, config.get("status")); node = _next(by_id, outgoing, node.id); continue
        if node_type == FlowNodeType.CONDITION.value:
            result = condition_result(db, conversation, config)
            logger.info("Telegram condition flow=%s node=%s result=%s", flow.id, node.id, result)
            node = _next(by_id, outgoing, node.id, "yes" if result else "no")
            continue
        if node_type == FlowNodeType.DELAY.value:
            logger.info("Telegram delay node skipped until scheduled resume support is implemented")
            node = _next(by_id, outgoing, node.id); continue
    ...
    session.status = "completed"; session.current_node_id = None; session.waiting_for = None; session.ended_at = datetime.utcnow(); session.updated_at = datetime.utcnow(); db.flush()
    return True


async def _run_flow(db, flow, conversation, inbound):
    nodes, by_id, outgoing = _graph(db, flow.id)
    trigger = next((n for n in nodes if _is_node_type(n, FlowNodeType.TRIGGER)), None)
    ...
    session.flow_id = flow.id; session.status = "active"; session.current_node_id = trigger.id; session.waiting_for = None
    session.last_inbound_message_id = inbound.id; session.started_at = now; session.updated_at = now; session.ended_at = None; db.flush()
...
async def _resume_waiting_session(db, conversation, inbound, session):
    ...
    target = db.scalar(select(FlowChannelTarget).where(FlowChannelTarget.flow_id == session.flow_id)) if flow else None
    if not flow or flow.status != FlowStatus.ACTIVE or not target or target.channel != "telegram" or flow.workspace_id != conversation.workspace_id:
        session.status = "reset"; session.current_node_id = None; session.waiting_for = None; session.ended_at = datetime.utcnow(); db.flush(); return False
    ...
    if not waiting_node or not _is_node_type(waiting_node, FlowNodeType.QUESTION):
        session.status = "failed"; session.waiting_for = None; session.ended_at = datetime.utcnow(); db.flush(); return False
    config = _json(waiting_node.config_json); valid, value, error = _validate_question_reply(config, inbound)
    session.last_inbound_message_id = inbound.id; session.updated_at = datetime.utcnow()
    if not valid:
        session.status = "waiting"; session.waiting_for = "reply"; await _send(db, conversation, error or "Please try again."); db.flush(); return True
    field_id = config.get("capture_field_id") or config.get("save_reply_field_id") or config.get("field_id")
    if field_id:
        stored = set_field(db, conversation, int(field_id), value)
        logger.info("Telegram question captured flow=%s node=%s field=%s stored=%s value=%r", flow.id, waiting_node.id, field_id, stored, value)
    session.status = "active"; session.waiting_for = None
    next_node = _next(by_id, outgoing, waiting_node.id, "next")
    if not next_node:
        session.status = "completed"; session.current_node_id = None; session.ended_at = datetime.utcnow(); db.flush(); return True
...
async def run_telegram_flows_for_inbound(db: Session, conversation: TelegramConversation, inbound: TelegramMessage) -> int:
    ...
    if existing and existing.status == "waiting":
        try:
            ...
            logger.exception("Telegram flow resume failed flow=%s conversation=%s", existing.flow_id, conversation.id); raise
    executed = 0
