...
const telegramStats = ref({ bots: 0, contacts: 0, conversations: 0, messages: 0, unread: 0 })
...
async function loadDashboard() {
  ...
  statsError.value = ''
  try {
    const [contacts, conversations, flows, tgStats] = await Promise.all([
  ...
  <main class="platform-shell">
      ...
      <nav class="platform-nav">
        ...
        <div class="nav-group">
          ...
          <div v-if="telegramOpen" class="subnav">
            <button :class="{ active: section === 'telegram' && telegramView === 'inbox' }" @click="openTelegram('inbox')">Live Chat</button>
            <button :class="{ active: section === 'telegram' && telegramView === 'contacts' }" @click="openTelegram('contacts')">Contacts</button>
            <button v-if="canManageFlows" :class="{ active: section === 'telegram' && telegramView === 'flows' }" @click="openTelegram('flows')">Flows</button>
            <button v-if="currentUser.role === 'admin'" :class="{ active: section === 'telegram' && telegramView === 'settings' }" @click="openTelegram('settings')">Settings</button>
          </div>
        </div>
    ...
    <section v-if="section === 'dashboard'" class="dashboard-page">
      ...
      <div class="telegram-preview">
        <div class="channel-heading"><div class="channel-icon telegram">T</div><div><h2>Telegram Stats</h2><p>Activity received through your connected Telegram bots.</p></div></div>
        <div v-if="telegramStats.bots" class="stats-grid telegram-stats">
          <article><span>Bots</span><strong>{{ telegramStats.bots }}</strong><small>Connected bots</small></article>
          <article><span>Contacts</span><strong>{{ telegramStats.contacts }}</strong><small>Telegram users</small></article>
          <article><span>Conversations</span><strong>{{ telegramStats.conversations }}</strong><small>Telegram chats</small></article>
          <article><span>Messages</span><strong>{{ telegramStats.messages }}</strong><small>{{ telegramStats.unread }} unread chat(s)</small></article>
        </div>
        <div v-else class="empty-channel"><strong>Connect your first Telegram bot</strong><span>Once the webhook is registered, inbound Telegram users and messages will start appearing here automatically.</span><button @click="openTelegram('settings')">Open Telegram setup →</button></div>
      </div>
    </section>

    <section v-else-if="section === 'telegram'" class="telegram-page">
      <header><p class="dash-eyebrow">Telegram</p><h1>{{ telegramView === 'inbox' ? 'Live Chat' : telegramView.charAt(0).toUpperCase() + telegramView.slice(1) }}</h1><p v-if="telegramView==='settings'">Connect a Telegram bot, verify the token and register the webhook.</p><p v-else>Telegram {{ telegramView === 'inbox' ? 'conversations' : telegramView }} will be wired into this channel next.</p></header>
      <TelegramSettings v-if="telegramView === 'settings' && currentUser.role === 'admin'" />
      <div v-else class="telegram-scaffold"><div class="telegram-mark">T</div><h2>Telegram {{ telegramView === 'inbox' ? 'Live Chat' : telegramView.charAt(0).toUpperCase() + telegramView.slice(1) }}</h2><p>The Bot API transport is now being connected. Once a bot receives its first message, this screen is the next piece we’ll turn live.</p><button class="telegram-setup-link" @click="openTelegram('settings')">Go to Telegram Settings</button><span>v0.2.0 · Telegram Core</span></div>
    </section>
