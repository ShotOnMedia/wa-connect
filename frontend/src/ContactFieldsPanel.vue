...
const form = reactive({ key:'', label:'', field_type:'text', options:'', required:false, active:true, protected:false, sort_order:0 })
...
const writableFields=computed(()=>customFields.value.filter(f=>f.writable!==false))
...
function startLiveRefresh(){if(refreshTimer)window.clearInterval(refreshTimer);refreshTimer=null;if(props.compact&&props.contact)refreshTimer=window.setInterval(()=>{if(!savingAll.value&&!document.hidden)loadValues({silent:true})},10000)}
...
<section v-if="contact" class="contact-section custom-fields-section" :class="{compact}">
  ...
  <template v-else>
    ...
    <div v-if="customFields.length" class="custom-title"><b>Custom fields</b><span>Shared subscriber data</span></div>
    <form v-if="customFields.length" class="custom-fields-form" @submit.prevent="saveAll">
      <div class="custom-fields-grid">
        <label v-for="field in customFields" :key="field.id" :class="{wide:field.field_type==='textarea',protected:field.protected}">
          <span class="custom-field-label">{{field.label}} <i v-if="field.required">Required</i><i v-if="field.protected" class="protected-mark">Protected</i></span>
          <textarea v-if="field.field_type==='textarea'" v-model="values[field.id]" rows="3" :disabled="field.writable===false"/>
          <select v-else-if="field.field_type==='select'" v-model="values[field.id]" :disabled="field.writable===false"><option value="">— Select —</option><option v-for="option in field.options" :key="option" :value="option">{{option}}</option></select>
          <span v-else-if="field.field_type==='checkbox'" class="checkbox-field" :class="{disabled:field.writable===false}"><input v-model="values[field.id]" type="checkbox" :disabled="field.writable===false"/> <span>Yes</span></span>
          <input v-else v-model="values[field.id]" :disabled="field.writable===false" :type="field.field_type==='number'?'number':field.field_type==='date'?'date':field.field_type==='email'?'email':'text'"/>
          <small v-if="field.writable===false" class="readonly-note">View only for agents</small><small v-else-if="saving[field.id]">Saving…</small>
        </label>
      </div>
      <div class="custom-fields-actions"><span>Shared across WA Connect automations.</span><button v-if="writableFields.length" class="primary" :disabled="savingAll">{{savingAll?'Saving…':'Save fields'}}</button></div>
    </form>
