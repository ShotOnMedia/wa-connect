...
let busy=false
function phrases(value){const seen=new Set();return String(value||'').split(/\r?\n/).map(v=>v.trim()).filter(v=>{const k=v.toLowerCase();if(!v||seen.has(k))return false;seen.add(k);return true})}
...
function render(editor,input){const values=phrases(input.value),chips=editor.querySelector('.trigger-chips');chips.innerHTML=values.map((v,i)=>`<span>${escapeHtml(v)}<button type="button" data-index="${i}" title="Remove trigger">×</button></span>`).join('');editor.querySelector('.trigger-empty').hidden=values.length>0;chips.querySelectorAll('button').forEach(btn=>btn.onclick=()=>{const next=phrases(input.value);next.splice(Number(btn.dataset.index),1);dispatchValue(input,next.join('\n'));render(editor,input)})}
