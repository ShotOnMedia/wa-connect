...
const selected=ref(props.modelValue??''),creating=ref(false),label=ref(''),key=ref(''),fieldType=ref('text'),error=ref(''),saving=ref(false),keyTouched=ref(false)
...
async function create(){error.value='';const l=label.value.trim(),k=slug(key.value||label.value);if(!l){error.value='Field label is required.';return}if(!k){error.value='Field key is required.';return}saving.value=true;try{const created=await api.createContactField({label:l,key:k,field_type:fieldType.value,active:true});emit('field-created',created);selected.value=optionValue(created);emit('update:modelValue',selected.value);cancel()}catch(e){error.value=e.message||'Could not create custom field.'}finally{saving.value=false}}
</script>
...
<template>
  <div class="custom-field-selector">
    <select v-model="selected" @change="changed">
      ...
      <option v-for="field in fields" :key="field.id" :value="optionValue(field)">{{field.label||field.name||field.key}}</option>
