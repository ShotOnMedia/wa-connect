...
async function render(){if(busy)return;busy=true;try{
  const selected=await selectedTelegram()||await selectedWhatsApp();if(!selected){lastKey='';return}
  ...
  panel.querySelector('.lc-takeover')?.remove();root=document.createElement('section');root.className='lc-takeover'
  ...
  const anchor=panel.querySelector('.live-chat-extras,.tg-profile-actions,.tg-whatsapp-summary,.snapshot');if(anchor)anchor.insertAdjacentElement('afterend',root);else panel.appendChild(root)
  root.querySelector('.lc-takeover-btn').onclick=async e=>{const btn=e.currentTarget,action=human?'resume':'takeover';btn.disabled=true;btn.textContent=human?'Returning…':'Taking control…';try{await controlRequest(`/${channel}/${conversation.id}/${action}`,{method:'POST'});lastKey='';setTimeout(render,20)}catch(err){const msg=root.querySelector('.lc-takeover-message');msg.textContent=err.message;msg.hidden=false;btn.disabled=false;btn.textContent=human?'↪ Return to automation':'✋ Take control'}}
  lastKey=key
}catch(_){ }finally{busy=false}}
function schedule(){clearTimeout(timer);timer=setTimeout(render,100)}
export function installLiveChatTakeover(){const observer=new MutationObserver(schedule);observer.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class']});schedule();window.addEventListener('beforeunload',()=>observer.disconnect(),{once:true})}
