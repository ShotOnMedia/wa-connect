<script setup>
defineProps({draft:{type:Object,required:true},fields:{type:Array,default:()=>[]},direct:{type:Boolean,default:false}})
...
<label>JSON source field<select v-model="draft.dynamic_source_field_id"><option value="">— Select custom field —</option><option v-for="f in fields" :key="f.id" :value="f.id">{{f.label||f.name||f.key}} · %{{f.key}}%</option></select></label>
...
<label>Save selection to<select v-model="draft.dynamic_save_field_id"><option value="">— Do not save —</option><option v-for="f in fields" :key="f.id" :value="f.id">{{f.label||f.name||f.key}} · %{{f.key}}%</option></select></label>
