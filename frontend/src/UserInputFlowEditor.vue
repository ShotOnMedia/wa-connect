...
<template><div class="editor"><div class="info"><b>User Input Flow</b><span>Select a reusable Questionnaire to run its questions automatically in sequence. Leave the selector on Manual questions to keep using individually connected Question blocks.</span></div><label>Question source<select :value="selectedCampaign?.id||''" @change="choose($event.target.value)"><option value="">Manual questions connected in the flow</option><option v-for="c in campaigns" :key="c.id" :value="c.id">{{c.name}} · {{c.question_count}} questions</option></select><small v-if="loading">Loading questionnaires…</small><small v-else-if="error" class="error">{{error}}</small><small v-else-if="selectedCampaign">Runs <b>{{selectedCampaign.name}}</b> automatically. The Next output continues only after all {{selectedCampaign.question_count}} questions are answered.</small></label><template v-if="!selectedCampaign"><label>Submission name<input v-model="draft.campaign_name" placeholder="e.g. Customer Satisfaction"></label><label>Description<textarea v-model="draft.campaign_description" rows="3" placeholder="What is this input flow used for?"></textarea></label></template><template v-else><div class="campaign-card"><b>{{selectedCampaign.name}}</b><span>{{selectedCampaign.description||'Reusable questionnaire'}}</span><small>Questionnaire ID: {{selectedCampaign.id}} · saved as the stable questionnaire reference.</small><code>%campaign.answers%</code><small>Complete response JSON remains available after the Questionnaire finishes. Individual answers are still available as <code>%input.answer_key%</code>.</small></div></template><label>Completion webhook URL<input v-model="draft.webhook_url" placeholder="https://example.com/webhook"></label><label class="check"><input v-model="draft.webhook_enabled" type="checkbox"> POST the consolidated response when the input flow completes</label><label>Thank-you message<textarea v-model="draft.thank_you_text" rows="3" placeholder="Thank you for your response!"></textarea></label><small>Questionnaire submissions snapshot the exact question text with every answer, so later questionnaire edits do not rewrite historic responses.</small></div></template>
