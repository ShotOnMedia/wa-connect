...
watch(()=>props.draft.capture_field_id,(id)=>{
  if(answerKey.value||!id)return
  const field=props.fields.find(item=>String(item.id)===String(id))
  if(field?.key)answerKey.value=field.key
})
...
<label class="answer-key">Answer key / variable name<input v-model="answerKey" placeholder="e.g. name or product"><small class="help">Use a short stable key. This answer becomes available to later blocks as <code>{{answerVariable}}</code>. If you choose a custom field below, its key is suggested automatically when this is blank.</small></label>
<label>Expected reply type<select v-model="draft.reply_type"><optgroup label="Text & data"><option value="text">Text</option><option value="email">Email address</option><option value="phone">Phone number (typed)</option><option value="telegram_phone">Telegram — Share phone number</option><option value="date">Date</option><option value="number">Number</option><option value="integer">Whole number</option></optgroup><optgroup label="Media"><option value="image">Photo / image</option><option value="audio">Audio / voice note</option><option value="video">Video</option><option value="document">Document / file</option><option value="sticker">Sticker</option><option value="media">Any media</option></optgroup></select><small class="help" v-if="isMedia">The flow remains waiting until the contact sends the selected media type. “Any media” accepts a photo, video, audio/voice note, document or sticker.</small><small class="help telegram-help" v-if="isTelegramPhone">Telegram shows a native <b>Share phone number</b> button. The supplied number is automatically stored in the system Subscriber ID field and the flow continues after Telegram returns the contact.</small></label>
...
<label v-if="!isTelegramPhone">Save valid reply to<CustomFieldSelector v-model="draft.capture_field_id" :fields="fields" value-mode="id" empty-label="Do not save reply" @field-created="emit('field-created',$event)"/><small class="help" v-if="isMedia">Media replies store stable channel metadata so the original media can be identified and reused later. Telegram stores the file ID, message IDs, filename/MIME details where available and caption; WhatsApp retains its channel media metadata.</small><small class="help" v-else>Optional. Saving to a custom field makes the value persistent on the subscriber as well as available through the User Input submission.</small></label>
<div v-else class="native-button-preview"><span>Telegram button</span><b>📱 Share phone number</b><small>Subscriber ID is populated automatically; no custom field is required.</small></div>
