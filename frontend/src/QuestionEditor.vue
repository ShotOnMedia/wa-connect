...
<label>Expected reply type<select v-model="draft.reply_type"><optgroup label="Text & data"><option value="text">Text</option><option value="email">Email address</option><option value="phone">Phone number</option><option value="date">Date</option><option value="number">Number</option><option value="integer">Whole number</option></optgroup><optgroup label="Media"><option value="image">Photo / image</option><option value="audio">Audio / voice note</option><option value="video">Video</option><option value="document">Document / file</option><option value="sticker">Sticker</option><option value="media">Any media</option></optgroup></select><small class="help" v-if="isMedia">The flow remains waiting until the contact sends this media type.</small></label>
...
<label>Save valid reply to<CustomFieldSelector v-model="draft.capture_field_id" :fields="fields" value-mode="id" empty-label="Do not save reply" @field-created="emit('field-created',$event)"/><small class="help">Media fields store stable WhatsApp media metadata, including the Meta media ID.</small></label>
